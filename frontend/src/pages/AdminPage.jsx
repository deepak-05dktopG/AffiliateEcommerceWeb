...
const AdminPage = () => {
    ...
    const confirmProductUpdate = () => {
        if (window.confirm(`Are you sure want to update ?`)) {
            ...
            axios.put(`http://localhost:5000/api/products/items/${productUpdate.edit_id}`, {
    ...
    const confirmUpdate = () => {
        if (window.confirm(`Are you sure want to update ?`)) {
            ...
            axios.put(`http://localhost:5000/api/categories/category/${update.edit_id}`, {
    ...
    const handleReply = async (message) => {
        ...
        if (replyText) {
            try {
                const res = await fetch(`http://localhost:5000/api/contact/reply/${message._id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email: message.email,
                        ...
                        <tbody>
                            {filteredProducts.slice().reverse().map((p, idx) => (
                                ...
                                productUpdate.status ? (
                                    <tr style={{ fontSize: '50px', wordBreak: 'break-all' }} className='' key={p._id}>
                                        <td>{idx + 1}</td>
                                        <td>
                                            <strong>
